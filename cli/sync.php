...
/**
 * CLI update for token enrolments, use for debugging or immediate update
 * of all courses.
 *
 * Notes:
...
if ($options['help']) {
    $help =
        "Execute token course enrol updates.
...
Example:
\$ sudo -u www-data /usr/bin/php enrol/token/cli/sync.php
";
...
if (!enrol_is_enabled('token')) {
    cli_error('enrol_token plugin is disabled, synchronisation stopped', 2);
}
